...
def main():
    ...
    # when entering the route, page will go to home page.
...
def info():
    ...
    # get information about class timetable
    cursor = getCursor()
    ...
    # sorted class time table by week day.
    sort_timetable = sorted(timetables, key=lambda x: weekday_order[x[0]])

    #get intruction about each class
    cursor = getCursor()
    ...
    #get intruction about each session
    cursor = getCursor()
    ...
    #get information about each therapist
    cursor = getCursor()
    cursor.execute('SELECT * from therapist ')
    therapists = cursor.fetchall()
    
    # route will jump to different information pages.
